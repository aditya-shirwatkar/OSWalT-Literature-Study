...
* **Problem :** Essential amount of airflow is blocked by the components that restrict the overall productive thrust. Around 44% of thrust loss is observed.
**Proposed Solution by the Authors :** None

* **Problem :** During flight the acoustic noise, the air turbulence from the propulsion system and the unstable power supply, which is caused by the current draw from the motors, had a major affect on the sensor measurements. For this reason, both ultrasonic modules and the localisation for altitude control were not applied in real-time for feedback to the control system of the spherical UAV.
**Proposed Solution by the Authors :** Ultrasonic sensors will be replaced by two light-weight lidar units.

* **Problem :** the acoustic noise generated by the propellers provided far too much interference for the ultrasonic distance measurements to be reliable and
**Solution used by the Authors :** Altitude of the spherical UAV was controlled directly through pilot intervention.

* **Problem :** the spherical UAV is not very responsive to control inputs, it is very stable in flight (it does not move off its hover point very easily) and it has a slight negative roll attitude at hover. The non-linear model did not take an unknown source moment into consideration that was restoring the sphere to hover position during flight.
**Solution used by the Author :** The first, design solution was to alleviate the effects of the restoring moment by changing the centre of gravity of the sphere and then use individual PD controllers for the lateral and longitudinal dynamics, this would allow for insight into the system and tuning of the controller gains. The second solution was to include an integral gain to the control system as to overcome the restoring moment, this has the disadvantage of actuator saturation. The former solution was chosen, the reason being it forced actuator saturation too quickly, during the experimental process. Furthermore, based on the control theory of this system, there is no need for I controller in the rotational dynamics.

* **Problem :** The spherical UAV can still be seen to have slight negative roll attitude which has the effect of causing flap saturation when trying to achieve a positive roll.
**Proposed Solution by the Authors :** None

* **Problem :** The steady-state error in Lateral-longitudinal PD control is due to the lack of integral control and the inadequate actuation authority. 
**Proposed Solution by the Authors :** To enhance the authority of actuators (flap) should be shifted further down from the CG and be replaced by more efficient aerofoil designs in order to produce adequate moments.
